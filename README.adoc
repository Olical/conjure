...
You can trial run Conjure with the interactive `:ConjureSchool` tutorial without actually installing it! The only perquisites are `curl` and an up to date `nvim`.

[source,bash]
----
bash <(curl -fLo https://raw.githubusercontent.com/Olical/conjure/develop/scripts/school.sh)
----
