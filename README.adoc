 ...
 * Once installed, open a file of a supported file type such as `clojure`, `fennel` or `janet`.
 ...
 * Use Conjure's various evaluation tools to execute the code you're interested in such as: Form, file, buffer or even form at a Neovim mark.
 * Every result is kept in a log buffer that you can access at any time. You can open this in a window or get updates through the HUD.
