...
local function connect(_47_)
  local host = _47_["host"]
  local port = _47_["port"]
  local cb = _47_["cb"]
  local port_file_path = _47_["port_file_path"]
  local connect_opts = _47_["connect-opts"]
  if state.get("conn") then
    disconnect()
  else
  end
  ...
  local function _59_(msg)
    return ui["display-result"](msg)
  end
  return a.assoc(state.get(), "conn", a["merge!"](nrepl.connect(a.merge({host = host, port = port, ["on-failure"] = _49_, ["on-success"] = _50_, ["on-error"] = _51_, ["on-message"] = _53_, ["side-effect-callback"] = _56_, ["default-callback"] = _59_}, connect_opts)), {["seen-ns"] = {}, port_file_path = port_file_path}))
end
