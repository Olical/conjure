...
local function prep_code(s)
  ...
  return (str.join("\n", a.map(_22_, a.filter(_23_, str.split(replace_arrows(s), "\n")))) .. "\n")
end
...
M.unbatch = function(msgs)
  ...
  return str.join("", a.map(_26_, msgs))
end
M["eval-str"] = function(opts)
  local function _27_(repl)
    ...
    return repl.send(prep_code(opts.code), _28_, {["batch?"] = true})
  end
  ...
  if state("repl") then
    ...
    local function _32_()
      ...
      local function _33_(repl)
        ...
        return repl.send(prep_code(M["initialise-repl-code"]), _34_, {batch = true})
      end
