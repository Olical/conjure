local _0_0
do
  local module_0_ = {}
  package.loaded["conjure.remote.netrepl"] = module_0_
  _0_0 = module_0_
end
local _local_0_ = {require("conjure.aniseed.core"), require("conjure.client"), require("conjure.log"), require("conjure.net"), require("conjure.remote.transport.netrepl")}
local a = _local_0_[1]
local client = _local_0_[2]
local log = _local_0_[3]
local net = _local_0_[4]
local trn = _local_0_[5]
local _2amodule_2a = _0_0
local _2amodule_name_2a = "conjure.remote.netrepl"
do local _ = ({nil, _0_0, {{}, nil, nil, nil}})[2] end
local send
do
  local v_0_
  local function send0(conn, msg, cb)
    log.dbg("send", msg)
    table.insert(conn.queue, 1, (cb or false))
    ; (conn.sock):write(trn.encode(msg))
    return nil
  end
  v_0_ = send0
  _0_0["send"] = v_0_
  send = v_0_
end
local connect
do
  local v_0_
  local function connect0(opts)
    local conn = {decode = trn.decoder(), queue = {}}
    local function handle_message(err, chunk)
      if (err or not chunk) then
        return opts["on-error"](err)
      else
        local function _1_(msg)
          log.dbg("receive", msg)
          local cb = table.remove(conn.queue)
          if cb then
            return cb(msg)
          end
        end
        return a["run!"](_1_, conn.decode(chunk))
      end
    end
    local function _1_(err)
      if err then
        return opts["on-failure"](err)
      else
        (conn.sock):read_start(client["schedule-wrap"](handle_message))
        return opts["on-success"]()
      end
    end
    conn = a.merge(conn, net.connect({cb = client["schedule-wrap"](_1_), host = opts.host, port = opts.port}))
    send(conn, (opts.name or "Conjure"))
    return conn
  end
  v_0_ = connect0
  _0_0["connect"] = v_0_
  connect = v_0_
end
return nil