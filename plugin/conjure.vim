" From: http://vim.wikia.com/wiki/Act_on_text_objects_with_custom_functions
...
function! MapAction(algorithm, key)
  exe 'nnoremap <buffer> <silent> <Plug>actions'    .a:algorithm.' :<C-U>call <SID>ActionSetup("'.a:algorithm.'")<CR>g@'
  exe 'xnoremap <buffer> <silent> <Plug>actions'    .a:algorithm.' :<C-U>call <SID>DoAction("'.a:algorithm.'",visualmode())<CR>'
  exe 'nnoremap <buffer> <silent> <Plug>actionsLine'.a:algorithm.' :<C-U>call <SID>DoAction("'.a:algorithm.'",v:count1)<CR>'
...
" TODO Improve and make optional.
function! s:Eval(code)
    call conjure#eval(a:code, expand("%"))
    return a:code
endfunction

function! s:Doc(code)
    call conjure#doc(a:code, expand("%"))
    return a:code
endfunction
...
autocmd FileType clojure call MapAction('Eval', '<localleader>re')
autocmd FileType clojure call MapAction('Doc', '<localleader>rd')
